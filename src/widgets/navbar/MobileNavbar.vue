...
watch(() => route.name, () => nextTick(updateDot));
</script>

<template>
  <nav class="fixed bottom-0 left-0 right-0 z-40 border-t border-[var(--color-border)] bg-[var(--color-surface)]">
    <div class="relative flex items-center justify-around h-16">
      ...
      <router-link
        ...
        :key="item.name"
        :ref="(el) => { if (el) itemRefs[i] = (el as any).$el ?? el }"
        :to="{ name: item.name }"
        :class="[
