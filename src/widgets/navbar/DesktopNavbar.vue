...
<template>
  <nav class="fixed top-0 left-0 right-0 z-40 h-16 border-b border-[var(--color-border)] bg-[var(--color-surface)]">
    <div class="mx-auto flex h-full max-w-6xl items-center justify-between px-4">
      <router-link to="/" class="flex items-center gap-2 text-[var(--color-text-primary)]">
        <LogoIcon size="2rem" />
        <span class="text-lg font-semibold text-[var(--color-text-primary)]" style="font-family: 'Space Grotesk', system-ui, sans-serif">
          PlaceBrain
        </span>
      </router-link>
      ...
      <div ref="navContainer" class="relative flex items-center gap-1">
        ...
        <router-link
          ...
          :key="item.name"
          :ref="(el) => { if (el) navRefs[i] = (el as any).$el ?? el }"
          :to="{ name: item.name }"
          :class="[
